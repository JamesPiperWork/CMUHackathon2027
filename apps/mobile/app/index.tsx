...
export default function Home() {
  ...
  const enabled = Object.entries(league?.settings.channels || state.consent.channels).filter(([, value]) => value).length;
  const ready = state.drafts.filter(draft => draft.locked && (!league || league.settings.channels[draft.channel])).length;
  ...
  return <View style={{ maxWidth: 720, alignSelf: "center", width: "100%", gap: 20, paddingTop: 8 }}>
    ...
    <Card testID="home-match" style={{ padding: width < 500 ? 23 : 30, gap: 24 }}>
      ...
      <View style={{ gap: 14 }}>
        <Txt muted style={{ textAlign: "center", fontSize: 13 }}>{preparing ? ready ? `${ready} of ${enabled} messages ready` : "Pick a hobby. We’ll help with the message." : `${6 - state.remaining} of 6 messages answered`}</Txt>
        <Button icon={paused && !done ? "settings" : done ? "sparkle" : preparing ? "hook" : "activity"} onPress={() => router.push(paused && !done ? "/settings" : done ? "/wrapped" : preparing ? "/draft" : "/activity")}>{paused && !done ? "Manage pause" : done ? "Watch Weekly Wrapped" : preparing ? ready ? "Finish your bait" : "Create your bait" : unread ? "Read messages" : "Open inbox"}</Button>
      </View>
    </Card>
