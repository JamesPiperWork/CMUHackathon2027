...
export default function League() {
  ...
  const [channels, setChannels] = useState<LeagueSettings['channels']>({ email: true, sms: true, voice: true });
  ...
  return <View style={{ maxWidth: 820, width: '100%', alignSelf: 'center' }}>
    <Title sub={`${league?.memberCount || 0} ${league?.memberCount === 1 ? "player" : "players"} · Week ${league?.currentWeek || 1}`}>{league?.name || state.league.name}</Title>
    <Row style={{ gap: 8, marginBottom: 24 }}>
      {([['/matchups', 'Matches', 'shield'], ['/chat', 'Chat', 'sms']] as const).map(([href, label, icon]) => <Button key={href} style={{ flex: 1, paddingHorizontal: 9 }} variant="secondary" icon={icon} onPress={() => router.push(href)}>{label}</Button>)}
    </Row>
    ...
    <View style={{ marginTop: 24, borderTopWidth: 1, borderTopColor: C.border }}>
      ...
      {showSettings && <View style={{ gap: 20, paddingBottom: 20 }}>
        ...
        <Card style={{ gap: 15 }}><Txt style={{ fontSize: 17, fontWeight: '700' }}>Rules</Txt><Txt muted style={{ fontSize: 13, lineHeight: 21 }}>Two casts each week, in any mix of enabled channels. One optional Spear per player, per league season.</Txt><Label>Difficulty</Label><Row style={{ gap: 7 }}>{(['rookie', 'standard', 'expert'] as const).map((value) => <Pressable key={value} accessibilityRole="button" accessibilityState={{ selected: difficulty === value, disabled: !commissioner }} disabled={!commissioner} onPress={() => { setDifficulty(value); setSaved(false); }} style={{ flex: 1, paddingVertical: 12, borderRadius: 7, borderWidth: 1, borderColor: difficulty === value ? C.teal : C.border, alignItems: 'center', backgroundColor: difficulty === value ? C.tealDark : C.panelDeep }}><Txt style={{ fontSize: 13, color: difficulty === value ? C.teal : C.muted }}>{value === 'rookie' ? 'Easy' : value === 'standard' ? 'Standard' : 'Hard'}</Txt></Pressable>)}</Row><Txt muted style={{ fontSize: 12, lineHeight: 19 }}>New message versions per cast: Easy 5 · Standard 3 · Hard 1.</Txt>
          {commissioner ? <><Label>Allowed channels</Label>{(['email', 'sms', 'voice'] as const).map(channel => <Toggle key={channel} label={channel === 'email' ? 'Email' : channel === 'sms' ? 'Text' : 'Voice'} value={channels[channel]} onChange={value => { setChannels({ ...channels, [channel]: value }); setSaved(false); }} />)}<Txt muted style={{ fontSize: 12, lineHeight: 20 }}>Each player still chooses their own contact preferences. Enabling a channel here does not opt them in.</Txt><Toggle label="Family-friendly themes" value={family} onChange={(value) => { setFamily(value); setSaved(false); }} /><Button disabled={!Object.values(channels).some(Boolean)} loading={busy} onPress={() => void safely(saveRules())}>{saved ? 'Saved' : 'Save rules'}</Button></> : <Txt muted style={{ fontSize: 13, lineHeight: 21 }}>The league organizer can edit these rules. {family ? 'Family-friendly themes are on.' : ''}</Txt>}
        </Card>
