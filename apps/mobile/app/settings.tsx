...
export default function Settings() {
  ...
  return <View style={{ maxWidth: 750, width: '100%', alignSelf: 'center' }}>
    ...
    <View style={{ marginTop: 22, borderTopWidth: 1, borderTopColor: C.border }}><Pressable accessibilityRole="button" accessibilityState={{ expanded: showDelivery }} onPress={() => setShowDelivery(!showDelivery)} style={{ paddingVertical: 18 }}><Txt muted style={{ fontSize: 14 }}>Delivery details {showDelivery ? '−' : '+'}</Txt></Pressable>{showDelivery && <View style={{ gap: 19, paddingBottom: 20 }}>{state.readiness.map((item) => <View key={item.channel} style={{ gap: 6 }}><Row style={{ justifyContent: 'space-between' }}><Txt style={{ fontSize: 14, fontWeight: '600' }}>{item.channel === 'sms' ? 'Text messages' : item.channel === 'voice' ? 'Voice calls' : 'Email'}</Txt><Txt style={{ fontSize: 12, color: item.status === 'simulated' || item.status === 'ready' ? C.teal : C.gold }}>{item.status}</Txt></Row><Txt muted style={{ fontSize: 12, lineHeight: 20 }}>{item.reason}</Txt></View>)}{state.mode === 'live' && <Txt muted style={{ fontSize: 12, lineHeight: 20 }}>Pausing cancels queued deliveries. A message already accepted by a carrier may still arrive.</Txt>}</View>}</View>
